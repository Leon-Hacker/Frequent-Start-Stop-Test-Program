from __future__ import annotations

import csv
import sys
import time
from dataclasses import dataclass
from datetime import datetime
from pathlib import Path
from queue import Empty, Queue
from typing import Optional

import serial
from serial.tools import list_ports

from PySide6.QtCore import QThread, QTimer, Qt, Signal, Slot
from PySide6.QtGui import QColor, QPalette
from PySide6.QtWidgets import (
    QApplication,
    QCheckBox,
    QComboBox,
    QDoubleSpinBox,
    QGridLayout,
    QGroupBox,
    QLabel,
    QMainWindow,
    QMessageBox,
    QPushButton,
    QPlainTextEdit,
    QSpinBox,
    QStatusBar,
    QVBoxLayout,
    QWidget,
)

from relay_protocol import (
    ProtocolError,
    RelayStatus,
    build_read_status_frame,
    build_write_single_frame,
    format_hex,
    parse_status_reply,
    parse_write_single_reply,
)

from power_protocol import (
    PowerStatus,
    build_measure_current_command,
    build_measure_voltage_command,
    build_output_command,
    build_query_output_command,
    build_set_current_command,
    build_set_voltage_command,
    encode_command,
    parse_float_response,
    parse_output_state,
)


# =========================
# 基本参数
# =========================

RELAY_POLL_INTERVAL_MS = 200
RELAY_SERIAL_TIMEOUT_SECONDS = 0.5
WRITE_CONFIRM_ATTEMPTS = 3

POWER_POLL_INTERVAL_MS = 1000
POWER_SERIAL_TIMEOUT_SECONDS = 0.8
POWER_DEFAULT_BAUDRATE = 19200
POWER_QUERY_INTERVAL_SECONDS = 0.05


# =========================
# 继电器线程
# =========================

@dataclass(frozen=True)
class RelayCommand:
    kind: str
    relay: Optional[int] = None
    state: Optional[bool] = None


class RelaySerialThread(QThread):
    connected = Signal(str)
    disconnected = Signal()
    error = Signal(str)
    log = Signal(str)
    relay_command_confirmed = Signal(int, bool)
    status_changed = Signal(object)

    def __init__(self, port_name: str, baudrate: int, address: int) -> None:
        super().__init__()
        self._commands: Queue[RelayCommand] = Queue()
        self._running = True
        self._port_name = port_name
        self._baudrate = baudrate
        self._address = address
        self._serial: Optional[serial.Serial] = None

    def run(self) -> None:
        try:
            self._serial = serial.Serial(
                port=self._port_name,
                baudrate=self._baudrate,
                bytesize=serial.EIGHTBITS,
                parity=serial.PARITY_NONE,
                stopbits=serial.STOPBITS_ONE,
                timeout=RELAY_SERIAL_TIMEOUT_SECONDS,
                write_timeout=RELAY_SERIAL_TIMEOUT_SECONDS,
            )
        except serial.SerialException as exc:
            self._running = False
            self.error.emit(f"打开继电器串口失败：{exc}")
            self.disconnected.emit()
            return

        self.connected.emit(f"{self._port_name} @ {self._baudrate}")
        self._loop()

    def stop(self) -> None:
        self._running = False
        self._commands.put(RelayCommand("stop"))

    def request_status(self) -> None:
        if self._commands.empty():
            self._commands.put(RelayCommand("read"))

    def set_relay(self, relay: int, state: bool) -> None:
        self._commands.put(RelayCommand("write", relay=relay, state=state))

    def _loop(self) -> None:
        while self._running:
            try:
                command = self._commands.get(timeout=0.05)
            except Empty:
                continue

            if command.kind == "stop":
                break

            if command.kind == "read":
                self._read_status()

            elif command.kind == "write" and command.relay is not None:
                self._write_relay(command.relay, bool(command.state))

        self._close_serial()
        self.disconnected.emit()

    def _close_serial(self) -> None:
        if self._serial is not None:
            try:
                self._serial.close()
            except serial.SerialException as exc:
                self.error.emit(f"关闭继电器串口失败：{exc}")
            finally:
                self._serial = None

    def _write_and_read(self, request: bytes, response_size: int) -> bytes:
        if self._serial is None or not self._serial.is_open:
            raise serial.SerialException("继电器串口未打开")

        self._serial.reset_input_buffer()
        self._serial.write(request)
        self._serial.flush()
        self.log.emit(f"TX  {format_hex(request)}")

        response = self._serial.read(response_size)

        if response:
            self.log.emit(f"RX  {format_hex(response)}")

        if len(response) != response_size:
            raise TimeoutError(
                f"响应超时：期望 {response_size} 字节，收到 {len(response)} 字节"
            )

        return response

    def _read_status(self, emit_errors: bool = True) -> Optional[RelayStatus]:
        try:
            request = build_read_status_frame(self._address)
            response = self._write_and_read(request, 15)
            status = parse_status_reply(response, self._address)
        except (ProtocolError, TimeoutError, serial.SerialException) as exc:
            if emit_errors:
                self.error.emit(f"读取继电器状态失败：{exc}")
            return None

        self.status_changed.emit(status)
        return status

    def _write_relay(self, relay: int, state: bool) -> None:
        action = "打开" if state else "关闭"
        last_error = ""

        for attempt in range(1, WRITE_CONFIRM_ATTEMPTS + 1):
            try:
                request = build_write_single_frame(self._address, relay, state)
                response = self._write_and_read(request, 10)
                parse_write_single_reply(response, self._address, relay, state)

                self.relay_command_confirmed.emit(relay, state)
                self._read_status(emit_errors=False)
                return

            except (ProtocolError, TimeoutError, serial.SerialException, ValueError) as exc:
                last_error = str(exc)

            if attempt < WRITE_CONFIRM_ATTEMPTS:
                self.log.emit(
                    f"INFO 第 {relay} 路{action}未确认，正在重试 "
                    f"({attempt + 1}/{WRITE_CONFIRM_ATTEMPTS})"
                )

        self.error.emit(
            f"控制继电器第 {relay} 路失败：已重试 {WRITE_CONFIRM_ATTEMPTS} 次，"
            f"未收到正确应答。{last_error}"
        )


# =========================
# 电源线程
# =========================

@dataclass(frozen=True)
class PowerCommand:
    kind: str
    voltage: Optional[float] = None
    current: Optional[float] = None
    output_on: Optional[bool] = None


class PowerSerialThread(QThread):
    connected = Signal(str)
    disconnected = Signal()
    error = Signal(str)
    log = Signal(str)
    status_changed = Signal(object)

    voltage_set_confirmed = Signal(float)
    current_set_confirmed = Signal(float)
    output_confirmed = Signal(bool)

    def __init__(self, port_name: str, baudrate: int) -> None:
        super().__init__()
        self._commands: Queue[PowerCommand] = Queue()
        self._running = True
        self._port_name = port_name
        self._baudrate = baudrate
        self._serial: Optional[serial.Serial] = None

    def run(self) -> None:
        try:
            self._serial = serial.Serial(
                port=self._port_name,
                baudrate=self._baudrate,
                bytesize=serial.EIGHTBITS,
                parity=serial.PARITY_NONE,
                stopbits=serial.STOPBITS_ONE,
                timeout=POWER_SERIAL_TIMEOUT_SECONDS,
                write_timeout=POWER_SERIAL_TIMEOUT_SECONDS,
            )
        except serial.SerialException as exc:
            self._running = False
            self.error.emit(f"打开电源串口失败：{exc}")
            self.disconnected.emit()
            return

        self.connected.emit(f"{self._port_name} @ {self._baudrate}")
        self._loop()

    def stop(self) -> None:
        self._running = False
        self._commands.put(PowerCommand("stop"))

    def request_status(self) -> None:
        if self._commands.empty():
            self._commands.put(PowerCommand("read_status"))

    def set_voltage(self, voltage: float) -> None:
        self._commands.put(PowerCommand("set_voltage", voltage=voltage))

    def set_current(self, current: float) -> None:
        self._commands.put(PowerCommand("set_current", current=current))

    def set_output(self, output_on: bool) -> None:
        self._commands.put(PowerCommand("set_output", output_on=output_on))

    def set_all_and_start(self, voltage: float, current: float) -> None:
        self._commands.put(
            PowerCommand(
                "set_all_and_start",
                voltage=voltage,
                current=current,
                output_on=True,
            )
        )

    def _loop(self) -> None:
        while self._running:
            try:
                command = self._commands.get(timeout=0.05)
            except Empty:
                continue

            if command.kind == "stop":
                break

            if command.kind == "read_status":
                self._read_status()

            elif command.kind == "set_voltage" and command.voltage is not None:
                self._set_voltage(command.voltage)

            elif command.kind == "set_current" and command.current is not None:
                self._set_current(command.current)

            elif command.kind == "set_output" and command.output_on is not None:
                self._set_output(command.output_on)

            elif (
                command.kind == "set_all_and_start"
                and command.voltage is not None
                and command.current is not None
            ):
                self._set_all_and_start(command.voltage, command.current)

        self._close_serial()
        self.disconnected.emit()

    def _close_serial(self) -> None:
        if self._serial is not None:
            try:
                self._serial.close()
            except serial.SerialException as exc:
                self.error.emit(f"关闭电源串口失败：{exc}")
            finally:
                self._serial = None

    def _write_command(self, command: str) -> None:
        if self._serial is None or not self._serial.is_open:
            raise serial.SerialException("电源串口未打开")

        self._serial.reset_input_buffer()
        self._serial.write(encode_command(command))
        self._serial.flush()
        self.log.emit(f"TX  {command}")

    def _query_command(self, command: str) -> str:
        if self._serial is None or not self._serial.is_open:
            raise serial.SerialException("电源串口未打开")

        self._serial.reset_input_buffer()
        self._serial.write(encode_command(command))
        self._serial.flush()
        self.log.emit(f"TX  {command}")

        response = self._serial.readline().decode("ascii", errors="replace").strip()

        if not response:
            raise TimeoutError("响应超时：未收到返回值")

        self.log.emit(f"RX  {response}")
        return response

    def _read_status(self, emit_errors: bool = True) -> Optional[PowerStatus]:
        try:
            current_response = self._query_command(build_measure_current_command())
            voltage_response = self._query_command(build_measure_voltage_command())
            output_response = self._query_command(build_query_output_command())

            measured_current = parse_float_response(current_response, "电流")
            measured_voltage = parse_float_response(voltage_response, "电压")
            output_on = parse_output_state(output_response)

        except (TimeoutError, serial.SerialException, ValueError, UnicodeError) as exc:
            if emit_errors:
                self.error.emit(f"读取电源状态失败：{exc}")
            return None

        status = PowerStatus(
            measured_voltage=measured_voltage,
            measured_current=measured_current,
            output_on=output_on,
        )

        self.status_changed.emit(status)
        return status

    def _set_voltage(self, voltage: float) -> None:
        try:
            self._write_command(build_set_voltage_command(voltage))
            self.voltage_set_confirmed.emit(voltage)
            self._read_status(emit_errors=False)
        except (TimeoutError, serial.SerialException, ValueError, UnicodeError) as exc:
            self.error.emit(f"设置电压失败：{exc}")

    def _set_current(self, current: float) -> None:
        try:
            self._write_command(build_set_current_command(current))
            self.current_set_confirmed.emit(current)
            self._read_status(emit_errors=False)
        except (TimeoutError, serial.SerialException, ValueError, UnicodeError) as exc:
            self.error.emit(f"设置电流失败：{exc}")

    def _set_output(self, output_on: bool) -> None:
        try:
            self._write_command(build_output_command(output_on))

            # 这里保留极短等待，避免部分电源刚写完输出状态就立刻查询导致返回不稳定
            time.sleep(POWER_QUERY_INTERVAL_SECONDS)

            self.output_confirmed.emit(output_on)
            self._read_status(emit_errors=False)

        except (TimeoutError, serial.SerialException, ValueError, UnicodeError) as exc:
            self.error.emit(f"设置输出状态失败：{exc}")

    def _set_all_and_start(self, voltage: float, current: float) -> None:
        try:
            self._write_command(build_set_voltage_command(voltage))
            time.sleep(POWER_QUERY_INTERVAL_SECONDS)
            self.voltage_set_confirmed.emit(voltage)

            self._write_command(build_set_current_command(current))
            time.sleep(POWER_QUERY_INTERVAL_SECONDS)
            self.current_set_confirmed.emit(current)

            self._write_command(build_output_command(True))
            time.sleep(POWER_QUERY_INTERVAL_SECONDS)
            self.output_confirmed.emit(True)

            self._read_status(emit_errors=False)

        except (TimeoutError, serial.SerialException, ValueError, UnicodeError) as exc:
            self.error.emit(f"设置并开启电源失败：{exc}")


# =========================
# 状态指示器
# =========================

class RelayIndicator(QLabel):
    def __init__(self, relay_name: str) -> None:
        super().__init__("未知")
        self._relay_name = relay_name
        self.setAlignment(Qt.AlignCenter)
        self.setMinimumHeight(36)
        self.setAutoFillBackground(True)
        self.set_status(None)

    def set_status(self, is_on: Optional[bool]) -> None:
        if is_on is None:
            text = f"{self._relay_name}：未知"
            color = QColor("#9ca3af")
        elif is_on:
            text = f"{self._relay_name}：已打开"
            color = QColor("#16a34a")
        else:
            text = f"{self._relay_name}：已关闭"
            color = QColor("#6b7280")

        palette = self.palette()
        palette.setColor(QPalette.Window, color)
        palette.setColor(QPalette.WindowText, QColor("white"))
        self.setPalette(palette)
        self.setText(text)


class PowerIndicator(QLabel):
    def __init__(self, title: str) -> None:
        super().__init__()
        self._title = title
        self.setAlignment(Qt.AlignCenter)
        self.setMinimumHeight(42)
        self.setAutoFillBackground(True)
        self.set_status(None)

    def set_status(self, output_on: Optional[bool]) -> None:
        if output_on is None:
            text = f"{self._title}：未知"
            color = QColor("#9ca3af")
        elif output_on:
            text = f"{self._title}：运行中"
            color = QColor("#16a34a")
        else:
            text = f"{self._title}：已停止"
            color = QColor("#6b7280")

        palette = self.palette()
        palette.setColor(QPalette.Window, color)
        palette.setColor(QPalette.WindowText, QColor("white"))
        self.setPalette(palette)
        self.setText(text)


# =========================
# 主窗口
# =========================

class MainWindow(QMainWindow):
    MAX_LOG_MESSAGES = 80

    def __init__(self) -> None:
        super().__init__()

        self.setWindowTitle("继电器 + 电源一体化串口控制器")
        self.resize(980, 900)

        self._relay_thread: Optional[RelaySerialThread] = None
        self._power_thread: Optional[PowerSerialThread] = None

        self._relay_connected = False
        self._power_connected = False

        self._log_messages: list[str] = []
        self._log_file_path = self._init_log_file()

        # 继电器轮询
        self._relay_poll_timer = QTimer(self)
        self._relay_poll_timer.setInterval(RELAY_POLL_INTERVAL_MS)
        self._relay_poll_timer.timeout.connect(self.request_relay_status)

        # 电源轮询
        self._power_poll_timer = QTimer(self)
        self._power_poll_timer.setInterval(POWER_POLL_INTERVAL_MS)
        self._power_poll_timer.timeout.connect(self.request_power_status)

        # 继电器第 1 路自动循环
        self._relay_cycle_timer = QTimer(self)
        self._relay_cycle_timer.setSingleShot(True)
        self._relay_cycle_timer.timeout.connect(self._on_relay_cycle_timeout)

        self._relay_countdown_timer = QTimer(self)
        self._relay_countdown_timer.setInterval(1000)
        self._relay_countdown_timer.timeout.connect(self._on_relay_countdown_tick)

        self._relay_auto_cycle_enabled = False
        self._relay_cycle_state = "wait_open"
        self._relay_countdown_remaining = 0

        # 电源定时通电
        self._power_timed_timer = QTimer(self)
        self._power_timed_timer.setInterval(1000)
        self._power_timed_timer.timeout.connect(self._on_power_timed_tick)

        self._power_timed_remaining = 0
        self._power_timed_active = False
        self._power_timed_csv_path: Optional[Path] = None
        self._power_timed_writer: Optional[csv.writer] = None
        self._power_timed_file: Optional[object] = None

        self._build_ui()
        self.refresh_all_ports()
        self._set_relay_connected(False)
        self._set_power_connected(False)

    # =========================
    # UI
    # =========================

    def _build_ui(self) -> None:
        root = QWidget(self)
        layout = QVBoxLayout(root)

        relay_group = self._build_relay_group()
        power_group = self._build_power_group()
        log_group = self._build_log_group()

        layout.addWidget(relay_group)
        layout.addWidget(power_group)
        layout.addWidget(log_group, stretch=1)

        self.setCentralWidget(root)
        self.setStatusBar(QStatusBar(self))
        self.statusBar().showMessage("未连接")

    def _build_relay_group(self) -> QGroupBox:
        group = QGroupBox("2 路继电器控制")
        layout = QGridLayout(group)

        self.relay_port_combo = QComboBox()
        self.relay_refresh_button = QPushButton("刷新串口")
        self.relay_refresh_button.clicked.connect(self.refresh_all_ports)

        self.relay_baud_combo = QComboBox()
        self.relay_baud_combo.addItems(["9600", "115200", "19200", "38400", "57600"])
        self.relay_baud_combo.setCurrentText("9600")

        self.relay_address_spin = QSpinBox()
        self.relay_address_spin.setRange(0, 255)
        self.relay_address_spin.setValue(1)

        self.relay_connect_button = QPushButton("打开继电器串口")
        self.relay_connect_button.clicked.connect(self.toggle_relay_connection)

        self.relay_auto_poll_check = QCheckBox("自动轮询")
        self.relay_auto_poll_check.setChecked(True)
        self.relay_auto_poll_check.toggled.connect(self._sync_relay_poll_timer)

        self.relay_manual_refresh_button = QPushButton("手动刷新")
        self.relay_manual_refresh_button.clicked.connect(self.request_relay_status)

        self.relay1_indicator = RelayIndicator("第 1 路")
        self.relay2_indicator = RelayIndicator("第 2 路")

        self.relay_buttons: list[QPushButton] = []

        self.relay1_on_button = self._relay_button("第 1 路打开", 1, True)
        self.relay1_off_button = self._relay_button("第 1 路关闭", 1, False)
        self.relay2_on_button = self._relay_button("第 2 路打开", 2, True)
        self.relay2_off_button = self._relay_button("第 2 路关闭", 2, False)

        self.relay_t1_spin = QSpinBox()
        self.relay_t1_spin.setRange(1, 24 * 3600)
        self.relay_t1_spin.setValue(10)
        self.relay_t1_spin.setSuffix(" s")

        self.relay_t2_spin = QSpinBox()
        self.relay_t2_spin.setRange(1, 24 * 3600)
        self.relay_t2_spin.setValue(2)
        self.relay_t2_spin.setSuffix(" s")

        self.relay_cycle_button = QPushButton("开始第 1 路自动循环")
        self.relay_cycle_button.setCheckable(True)
        self.relay_cycle_button.clicked.connect(self._toggle_relay_auto_cycle)

        row = 0
        layout.addWidget(QLabel("继电器串口"), row, 0)
        layout.addWidget(self.relay_port_combo, row, 1)
        layout.addWidget(self.relay_refresh_button, row, 2)
        layout.addWidget(self.relay_connect_button, row, 3)

        row += 1
        layout.addWidget(QLabel("波特率"), row, 0)
        layout.addWidget(self.relay_baud_combo, row, 1)
        layout.addWidget(QLabel("地址"), row, 2)
        layout.addWidget(self.relay_address_spin, row, 3)

        row += 1
        layout.addWidget(self.relay_auto_poll_check, row, 0)
        layout.addWidget(self.relay_manual_refresh_button, row, 1)

        row += 1
        layout.addWidget(self.relay1_indicator, row, 0, 1, 2)
        layout.addWidget(self.relay2_indicator, row, 2, 1, 2)

        row += 1
        layout.addWidget(self.relay1_on_button, row, 0)
        layout.addWidget(self.relay1_off_button, row, 1)
        layout.addWidget(self.relay2_on_button, row, 2)
        layout.addWidget(self.relay2_off_button, row, 3)

        row += 1
        layout.addWidget(QLabel("T1 打开前等待"), row, 0)
        layout.addWidget(self.relay_t1_spin, row, 1)
        layout.addWidget(QLabel("T2 打开后保持"), row, 2)
        layout.addWidget(self.relay_t2_spin, row, 3)

        row += 1
        layout.addWidget(self.relay_cycle_button, row, 0, 1, 4)

        return group

    def _build_power_group(self) -> QGroupBox:
        group = QGroupBox("串口电源控制")
        layout = QGridLayout(group)

        self.power_port_combo = QComboBox()
        self.power_refresh_button = QPushButton("刷新串口")
        self.power_refresh_button.clicked.connect(self.refresh_all_ports)

        self.power_baud_combo = QComboBox()
        self.power_baud_combo.addItems(["19200", "9600", "115200", "38400", "57600"])
        self.power_baud_combo.setCurrentText(str(POWER_DEFAULT_BAUDRATE))

        self.power_connect_button = QPushButton("打开电源串口")
        self.power_connect_button.clicked.connect(self.toggle_power_connection)

        self.power_auto_poll_check = QCheckBox("实时监控")
        self.power_auto_poll_check.setChecked(True)
        self.power_auto_poll_check.toggled.connect(self._sync_power_poll_timer)

        self.power_manual_refresh_button = QPushButton("手动读取")
        self.power_manual_refresh_button.clicked.connect(self.request_power_status)

        self.voltage_spin = QDoubleSpinBox()
        self.voltage_spin.setRange(0.0, 1000.0)
        self.voltage_spin.setDecimals(3)
        self.voltage_spin.setSingleStep(0.1)
        self.voltage_spin.setSuffix(" V")

        self.current_spin = QDoubleSpinBox()
        self.current_spin.setRange(0.0, 1000.0)
        self.current_spin.setDecimals(3)
        self.current_spin.setSingleStep(0.1)
        self.current_spin.setSuffix(" A")

        self.set_voltage_button = QPushButton("设置电压")
        self.set_voltage_button.clicked.connect(self.set_power_voltage)

        self.set_current_button = QPushButton("设置电流")
        self.set_current_button.clicked.connect(self.set_power_current)

        self.apply_and_start_button = QPushButton("设置并开启")
        self.apply_and_start_button.clicked.connect(self.power_apply_and_start)

        self.output_on_button = QPushButton("开启输出")
        self.output_on_button.clicked.connect(lambda: self.set_power_output(True))

        self.output_off_button = QPushButton("停止输出")
        self.output_off_button.clicked.connect(lambda: self.set_power_output(False))

        self.power_indicator = PowerIndicator("电源输出")

        self.measured_voltage_label = QLabel("-- V")
        self.measured_current_label = QLabel("-- A")
        self.output_state_label = QLabel("未知")

        for label in (
            self.measured_voltage_label,
            self.measured_current_label,
            self.output_state_label,
        ):
            label.setAlignment(Qt.AlignCenter)
            label.setMinimumHeight(36)
            label.setStyleSheet("font-size: 18px; font-weight: 600;")

        self.duration_spin = QDoubleSpinBox()
        self.duration_spin.setRange(0.1, 86400.0)
        self.duration_spin.setDecimals(1)
        self.duration_spin.setSingleStep(0.5)
        self.duration_spin.setValue(10.0)
        self.duration_spin.setSuffix(" s")

        self.start_timed_button = QPushButton("开始定时通电")
        self.start_timed_button.setCheckable(True)
        self.start_timed_button.clicked.connect(self._toggle_power_timed_output)

        self.timed_status_label = QLabel("剩余时间：-- s")
        self.timed_status_label.setAlignment(Qt.AlignCenter)
        self.timed_status_label.setMinimumHeight(30)

        row = 0
        layout.addWidget(QLabel("电源串口"), row, 0)
        layout.addWidget(self.power_port_combo, row, 1)
        layout.addWidget(self.power_refresh_button, row, 2)
        layout.addWidget(self.power_connect_button, row, 3)

        row += 1
        layout.addWidget(QLabel("波特率"), row, 0)
        layout.addWidget(self.power_baud_combo, row, 1)
        layout.addWidget(self.power_auto_poll_check, row, 2)
        layout.addWidget(self.power_manual_refresh_button, row, 3)

        row += 1
        layout.addWidget(QLabel("设定电压"), row, 0)
        layout.addWidget(self.voltage_spin, row, 1)
        layout.addWidget(self.set_voltage_button, row, 2)
        layout.addWidget(self.apply_and_start_button, row, 3)

        row += 1
        layout.addWidget(QLabel("设定电流"), row, 0)
        layout.addWidget(self.current_spin, row, 1)
        layout.addWidget(self.set_current_button, row, 2)

        row += 1
        layout.addWidget(self.output_on_button, row, 1)
        layout.addWidget(self.output_off_button, row, 2)

        row += 1
        layout.addWidget(self.power_indicator, row, 0, 1, 4)

        row += 1
        layout.addWidget(QLabel("实测电压"), row, 0)
        layout.addWidget(self.measured_voltage_label, row, 1)
        layout.addWidget(QLabel("实测电流"), row, 2)
        layout.addWidget(self.measured_current_label, row, 3)

        row += 1
        layout.addWidget(QLabel("输出状态"), row, 0)
        layout.addWidget(self.output_state_label, row, 1)

        row += 1
        layout.addWidget(QLabel("通电时长"), row, 0)
        layout.addWidget(self.duration_spin, row, 1)
        layout.addWidget(self.start_timed_button, row, 2)
        layout.addWidget(self.timed_status_label, row, 3)

        return group

    def _build_log_group(self) -> QGroupBox:
        group = QGroupBox("综合通信日志")
        layout = QVBoxLayout(group)

        self.log_view = QPlainTextEdit()
        self.log_view.setReadOnly(True)

        self.clear_log_button = QPushButton("清空显示日志")
        self.clear_log_button.clicked.connect(self._clear_log)

        layout.addWidget(self.log_view)
        layout.addWidget(self.clear_log_button, alignment=Qt.AlignRight)

        return group

    def _relay_button(self, text: str, relay: int, state: bool) -> QPushButton:
        button = QPushButton(text)
        button.clicked.connect(lambda _checked=False, r=relay, s=state: self.set_relay(r, s))
        self.relay_buttons.append(button)
        return button

    # =========================
    # 串口刷新
    # =========================

    @Slot()
    def refresh_all_ports(self) -> None:
        relay_current = self.relay_port_combo.currentData() or self.relay_port_combo.currentText()
        power_current = self.power_port_combo.currentData() or self.power_port_combo.currentText()

        ports = list(list_ports.comports())

        self.relay_port_combo.clear()
        self.power_port_combo.clear()

        for port in ports:
            label = f"{port.device}  {port.description}"
            self.relay_port_combo.addItem(label, port.device)
            self.power_port_combo.addItem(label, port.device)

        if relay_current:
            index = self.relay_port_combo.findData(relay_current)
            if index >= 0:
                self.relay_port_combo.setCurrentIndex(index)

        if power_current:
            index = self.power_port_combo.findData(power_current)
            if index >= 0:
                self.power_port_combo.setCurrentIndex(index)

        if not ports:
            self.relay_port_combo.addItem("未发现串口", "")
            self.power_port_combo.addItem("未发现串口", "")

    # =========================
    # 继电器控制
    # =========================

    @Slot()
    def toggle_relay_connection(self) -> None:
        if self._relay_connected:
            self._disconnect_relay()
        else:
            self._connect_relay()

    def _connect_relay(self) -> None:
        port_name = self.relay_port_combo.currentData()

        if not port_name:
            QMessageBox.warning(self, "无法打开继电器串口", "请先选择可用串口。")
            return

        self._relay_thread = RelaySerialThread(
            port_name=port_name,
            baudrate=int(self.relay_baud_combo.currentText()),
            address=self.relay_address_spin.value(),
        )

        self._relay_thread.connected.connect(self._on_relay_connected)
        self._relay_thread.disconnected.connect(self._on_relay_disconnected)
        self._relay_thread.error.connect(self._on_error)
        self._relay_thread.log.connect(lambda msg: self._append_log(f"[继电器] {msg}"))
        self._relay_thread.relay_command_confirmed.connect(self._on_relay_command_confirmed)
        self._relay_thread.status_changed.connect(self._on_relay_status_changed)

        self._relay_thread.start()

        self.relay_connect_button.setEnabled(False)
        self.statusBar().showMessage("正在打开继电器串口...")

    def _disconnect_relay(self) -> None:
        self._relay_poll_timer.stop()
        self._stop_relay_auto_cycle()

        if self._relay_thread is not None:
            self._relay_thread.stop()

        self.statusBar().showMessage("正在关闭继电器串口...")

    @Slot(str)
    def _on_relay_connected(self, message: str) -> None:
        self._set_relay_connected(True)
        self._append_log(f"INFO 继电器已连接 {message}")
        self.statusBar().showMessage(f"继电器已连接 {message}")

        self._sync_relay_poll_timer()
        self.request_relay_status()

    @Slot()
    def _on_relay_disconnected(self) -> None:
        self._relay_poll_timer.stop()
        self._stop_relay_auto_cycle()
        self._set_relay_connected(False)

        self._append_log("INFO 继电器已断开")
        self.statusBar().showMessage("继电器未连接")

        if self._relay_thread is not None:
            self._relay_thread.wait(1000)
            self._relay_thread = None

    @Slot()
    def request_relay_status(self) -> None:
        if self._relay_thread is not None:
            self._relay_thread.request_status()

    @Slot(int, bool)
    def set_relay(self, relay: int, state: bool) -> None:
        if self._relay_thread is not None:
            self._relay_thread.set_relay(relay, state)

    @Slot(int, bool)
    def _on_relay_command_confirmed(self, relay: int, state: bool) -> None:
        action = "打开" if state else "关闭"
        message = f"继电器第 {relay} 路{action}成功，应答已确认"
        self._append_log(f"OK   {message}")
        self.statusBar().showMessage(message)

    @Slot(object)
    def _on_relay_status_changed(self, status: RelayStatus) -> None:
        self.relay1_indicator.set_status(status.relay1)
        self.relay2_indicator.set_status(status.relay2)

        self.statusBar().showMessage(
            f"继电器状态：第 1 路 {'开' if status.relay1 else '关'}，"
            f"第 2 路 {'开' if status.relay2 else '关'}"
        )

    @Slot()
    def _sync_relay_poll_timer(self) -> None:
        if self._relay_connected and self.relay_auto_poll_check.isChecked():
            self._relay_poll_timer.start()
        else:
            self._relay_poll_timer.stop()

    def _set_relay_connected(self, connected: bool) -> None:
        self._relay_connected = connected

        self.relay_connect_button.setEnabled(True)
        self.relay_connect_button.setText("关闭继电器串口" if connected else "打开继电器串口")

        self.relay_refresh_button.setEnabled(not connected)
        self.relay_port_combo.setEnabled(not connected)
        self.relay_baud_combo.setEnabled(not connected)
        self.relay_address_spin.setEnabled(not connected)

        self.relay_manual_refresh_button.setEnabled(connected)
        self.relay_auto_poll_check.setEnabled(connected)

        for button in self.relay_buttons:
            button.setEnabled(connected)

        self.relay_cycle_button.setEnabled(connected)

        if not connected:
            self.relay1_indicator.set_status(None)
            self.relay2_indicator.set_status(None)

    # =========================
    # 继电器自动循环
    # =========================

    def _toggle_relay_auto_cycle(self, checked: bool) -> None:
        if checked:
            self._start_relay_auto_cycle()
        else:
            self._stop_relay_auto_cycle()

    def _start_relay_auto_cycle(self) -> None:
        if not self._relay_connected:
            QMessageBox.warning(self, "继电器未连接", "请先打开继电器串口再启动自动循环。")
            self.relay_cycle_button.setChecked(False)
            return

        if self._relay_auto_cycle_enabled:
            return

        self._relay_auto_cycle_enabled = True
        self._relay_cycle_state = "wait_open"

        self._append_log(
            f"INFO 继电器第 1 路自动循环启动："
            f"T1={self.relay_t1_spin.value()}s, T2={self.relay_t2_spin.value()}s"
        )

        for button in self.relay_buttons:
            button.setEnabled(False)

        self._relay_countdown_remaining = int(self.relay_t1_spin.value())
        self.relay_cycle_button.setText(
            f"停止第 1 路自动循环 ({self._relay_countdown_remaining}s)"
        )
        self.relay_cycle_button.setChecked(True)

        self._relay_countdown_timer.start()
        self._relay_cycle_timer.start(self._relay_countdown_remaining * 1000)

    def _stop_relay_auto_cycle(self) -> None:
        if not self._relay_auto_cycle_enabled:
            return

        self._relay_auto_cycle_enabled = False
        self._relay_cycle_timer.stop()
        self._relay_countdown_timer.stop()
        self._relay_countdown_remaining = 0

        self._append_log("INFO 继电器第 1 路自动循环已停止")

        self.relay_cycle_button.setText("开始第 1 路自动循环")
        self.relay_cycle_button.setChecked(False)

        for button in self.relay_buttons:
            button.setEnabled(self._relay_connected)

    def _on_relay_countdown_tick(self) -> None:
        if not self._relay_auto_cycle_enabled:
            self._relay_countdown_timer.stop()
            return

        if self._relay_countdown_remaining > 0:
            self._relay_countdown_remaining -= 1
            self.relay_cycle_button.setText(
                f"停止第 1 路自动循环 ({self._relay_countdown_remaining}s)"
            )
        else:
            self._relay_countdown_timer.stop()

    def _on_relay_cycle_timeout(self) -> None:
        if not self._relay_auto_cycle_enabled:
            return

        if self._relay_cycle_state == "wait_open":
            self._append_log("AUTO 继电器第 1 路打开")
            self.set_relay(1, True)

            self._relay_cycle_state = "wait_close"
            self._relay_countdown_remaining = int(self.relay_t2_spin.value())

        else:
            self._append_log("AUTO 继电器第 1 路关闭")
            self.set_relay(1, False)

            self._relay_cycle_state = "wait_open"
            self._relay_countdown_remaining = int(self.relay_t1_spin.value())

        self.relay_cycle_button.setText(
            f"停止第 1 路自动循环 ({self._relay_countdown_remaining}s)"
        )
        self._relay_countdown_timer.start()
        self._relay_cycle_timer.start(self._relay_countdown_remaining * 1000)

    # =========================
    # 电源控制
    # =========================

    @Slot()
    def toggle_power_connection(self) -> None:
        if self._power_connected:
            self._disconnect_power()
        else:
            self._connect_power()

    def _connect_power(self) -> None:
        port_name = self.power_port_combo.currentData()

        if not port_name:
            QMessageBox.warning(self, "无法打开电源串口", "请先选择可用串口。")
            return

        self._power_thread = PowerSerialThread(
            port_name=port_name,
            baudrate=int(self.power_baud_combo.currentText()),
        )

        self._power_thread.connected.connect(self._on_power_connected)
        self._power_thread.disconnected.connect(self._on_power_disconnected)
        self._power_thread.error.connect(self._on_error)
        self._power_thread.log.connect(lambda msg: self._append_log(f"[电源] {msg}"))
        self._power_thread.status_changed.connect(self._on_power_status_changed)
        self._power_thread.voltage_set_confirmed.connect(self._on_voltage_set_confirmed)
        self._power_thread.current_set_confirmed.connect(self._on_current_set_confirmed)
        self._power_thread.output_confirmed.connect(self._on_output_confirmed)

        self._power_thread.start()

        self.power_connect_button.setEnabled(False)
        self.statusBar().showMessage("正在打开电源串口...")

    def _disconnect_power(self) -> None:
        self._power_poll_timer.stop()
        self._stop_power_timed_output(send_output_off=False)

        if self._power_thread is not None:
            self._power_thread.stop()

        self.statusBar().showMessage("正在关闭电源串口...")

    @Slot(str)
    def _on_power_connected(self, message: str) -> None:
        self._set_power_connected(True)
        self._append_log(f"INFO 电源已连接 {message}")
        self.statusBar().showMessage(f"电源已连接 {message}")

        self._sync_power_poll_timer()
        self.request_power_status()

    @Slot()
    def _on_power_disconnected(self) -> None:
        self._power_poll_timer.stop()
        self._stop_power_timed_output(send_output_off=False)
        self._set_power_connected(False)

        self._append_log("INFO 电源已断开")
        self.statusBar().showMessage("电源未连接")

        if self._power_thread is not None:
            self._power_thread.wait(1000)
            self._power_thread = None

    @Slot()
    def request_power_status(self) -> None:
        if self._power_thread is not None:
            self._power_thread.request_status()

    @Slot()
    def set_power_voltage(self) -> None:
        if self._power_thread is not None:
            self._power_thread.set_voltage(float(self.voltage_spin.value()))

    @Slot()
    def set_power_current(self) -> None:
        if self._power_thread is not None:
            self._power_thread.set_current(float(self.current_spin.value()))

    @Slot()
    def power_apply_and_start(self) -> None:
        if self._power_thread is not None:
            self._power_thread.set_all_and_start(
                voltage=float(self.voltage_spin.value()),
                current=float(self.current_spin.value()),
            )

    def set_power_output(self, output_on: bool) -> None:
        if self._power_thread is not None:
            self._power_thread.set_output(output_on)

    @Slot(float)
    def _on_voltage_set_confirmed(self, voltage: float) -> None:
        message = f"电源电压设定命令已发送：{voltage:.3f} V"
        self._append_log(f"OK   {message}")
        self.statusBar().showMessage(message)

    @Slot(float)
    def _on_current_set_confirmed(self, current: float) -> None:
        message = f"电源电流设定命令已发送：{current:.3f} A"
        self._append_log(f"OK   {message}")
        self.statusBar().showMessage(message)

    @Slot(bool)
    def _on_output_confirmed(self, output_on: bool) -> None:
        action = "开启" if output_on else "停止"
        message = f"电源输出{action}命令已发送"
        self._append_log(f"OK   {message}")
        self.statusBar().showMessage(message)

    @Slot(object)
    def _on_power_status_changed(self, status: PowerStatus) -> None:
        if status.measured_voltage is None:
            self.measured_voltage_label.setText("-- V")
        else:
            self.measured_voltage_label.setText(f"{status.measured_voltage:.3f} V")

        if status.measured_current is None:
            self.measured_current_label.setText("-- A")
        else:
            self.measured_current_label.setText(f"{status.measured_current:.3f} A")

        self.power_indicator.set_status(status.output_on)

        if status.output_on is True:
            state_text = "开启"
        elif status.output_on is False:
            state_text = "关闭"
        else:
            state_text = "未知"

        self.output_state_label.setText(state_text)

        if self._power_timed_active:
            self._record_power_timed_row(status)

        self.statusBar().showMessage(
            f"电源状态：U={self.measured_voltage_label.text()}，"
            f"I={self.measured_current_label.text()}，输出={state_text}"
        )

    @Slot()
    def _sync_power_poll_timer(self) -> None:
        if self._power_connected and self.power_auto_poll_check.isChecked():
            self._power_poll_timer.start()
        else:
            self._power_poll_timer.stop()

    def _set_power_connected(self, connected: bool) -> None:
        self._power_connected = connected

        self.power_connect_button.setEnabled(True)
        self.power_connect_button.setText("关闭电源串口" if connected else "打开电源串口")

        self.power_refresh_button.setEnabled(not connected)
        self.power_port_combo.setEnabled(not connected)
        self.power_baud_combo.setEnabled(not connected)

        self.power_manual_refresh_button.setEnabled(connected)
        self.power_auto_poll_check.setEnabled(connected)

        for widget in (
            self.voltage_spin,
            self.current_spin,
            self.set_voltage_button,
            self.set_current_button,
            self.apply_and_start_button,
            self.output_on_button,
            self.output_off_button,
            self.duration_spin,
            self.start_timed_button,
        ):
            widget.setEnabled(connected)

        if not connected:
            self.measured_voltage_label.setText("-- V")
            self.measured_current_label.setText("-- A")
            self.output_state_label.setText("未知")
            self.power_indicator.set_status(None)

    # =========================
    # 电源定时通电
    # =========================

    def _toggle_power_timed_output(self, checked: bool) -> None:
        if checked:
            self._start_power_timed_output()
        else:
            self._stop_power_timed_output(send_output_off=True)

    def _start_power_timed_output(self) -> None:
        if not self._power_connected or self._power_thread is None:
            QMessageBox.warning(self, "电源未连接", "请先打开电源串口再启动定时通电。")
            self.start_timed_button.setChecked(False)
            return

        if self._power_timed_active:
            return

        self._power_timed_active = True
        self._power_timed_remaining = int(self.duration_spin.value())

        self._open_power_timed_csv()

        if not self._power_timed_active:
            return

        self._append_log(
            f"INFO 电源定时通电启动：{self._power_timed_remaining} 秒，"
            f"电压 {self.voltage_spin.value():.3f} V，"
            f"电流 {self.current_spin.value():.3f} A"
        )

        self._power_thread.set_all_and_start(
            voltage=float(self.voltage_spin.value()),
            current=float(self.current_spin.value()),
        )

        self.start_timed_button.setText(f"停止定时通电 ({self._power_timed_remaining}s)")
        self.timed_status_label.setText(f"剩余时间：{self._power_timed_remaining} s")
        self._power_timed_timer.start()

    def _stop_power_timed_output(self, send_output_off: bool = True) -> None:
        if not self._power_timed_active:
            return

        self._power_timed_active = False
        self._power_timed_timer.stop()

        self.start_timed_button.setChecked(False)
        self.start_timed_button.setText("开始定时通电")
        self.timed_status_label.setText("剩余时间：-- s")

        self._append_log("INFO 电源定时通电停止")

        if self._power_timed_file is not None:
            try:
                self._power_timed_file.close()
            except Exception:
                pass

            self._power_timed_file = None
            self._power_timed_writer = None

        if send_output_off and self._power_thread is not None:
            self._power_thread.set_output(False)

    def _on_power_timed_tick(self) -> None:
        if not self._power_timed_active:
            return

        self._power_timed_remaining -= 1

        if self._power_timed_remaining <= 0:
            self.timed_status_label.setText("剩余时间：0 s")
            self._stop_power_timed_output(send_output_off=True)
            return

        self.timed_status_label.setText(f"剩余时间：{self._power_timed_remaining} s")
        self.start_timed_button.setText(f"停止定时通电 ({self._power_timed_remaining}s)")

    def _open_power_timed_csv(self) -> None:
        output_dir = Path(__file__).resolve().parent
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self._power_timed_csv_path = output_dir / f"power_output_{timestamp}.csv"

        try:
            self._power_timed_file = open(
                self._power_timed_csv_path,
                "w",
                encoding="utf-8",
                newline="",
            )
            self._power_timed_writer = csv.writer(self._power_timed_file)
            self._power_timed_writer.writerow(["timestamp", "voltage_V", "current_A"])

        except Exception as exc:
            self._append_log(f"ERR  无法创建电源 CSV 文件：{exc}")
            self._power_timed_active = False
            self.start_timed_button.setChecked(False)
            self.start_timed_button.setText("开始定时通电")
            self.timed_status_label.setText("剩余时间：-- s")

    def _record_power_timed_row(self, status: PowerStatus) -> None:
        if self._power_timed_active and self._power_timed_writer is not None:
            try:
                self._power_timed_writer.writerow(
                    [
                        datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-5],
                        status.measured_voltage if status.measured_voltage is not None else "",
                        status.measured_current if status.measured_current is not None else "",
                    ]
                )

                if self._power_timed_file is not None:
                    self._power_timed_file.flush()

            except Exception as exc:
                self._append_log(f"ERR  电源 CSV 写入失败：{exc}")

    # =========================
    # 日志
    # =========================

    def _init_log_file(self) -> Path:
        log_dir = Path(__file__).resolve().parent / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)

        log_filename = f"integrated_log_{datetime.now().strftime('%Y-%m-%d')}.txt"
        return log_dir / log_filename

    def _get_rotated_log_file(self) -> Path:
        log_dir = Path(__file__).resolve().parent / "logs"
        datepart = datetime.now().strftime("%Y-%m-%d")
        base_name = f"integrated_log_{datepart}"

        log_dir.mkdir(parents=True, exist_ok=True)

        candidates = sorted(log_dir.glob(f"{base_name}*.txt"))

        if not candidates:
            return log_dir / f"{base_name}.txt"

        last = candidates[-1]

        try:
            if last.stat().st_size < 10 * 1024 * 1024:
                return last
        except OSError:
            return log_dir / f"{base_name}.txt"

        stem = last.stem

        if stem == base_name:
            idx = 1
        else:
            parts = stem.rsplit("_", 1)
            if len(parts) == 2 and parts[0] == base_name and parts[1].isdigit():
                idx = int(parts[1]) + 1
            else:
                idx = 1

        return log_dir / f"{base_name}_{idx}.txt"

    def _save_log_to_file(self, message: str) -> None:
        try:
            self._log_file_path = self._get_rotated_log_file()

            with open(self._log_file_path, "a", encoding="utf-8") as file:
                file.write(message + "\n")

        except Exception as exc:
            print(f"保存日志文件失败: {exc}")

    @Slot(str)
    def _append_log(self, message: str) -> None:
        timestamp = time.strftime("%H:%M:%S")
        formatted_message = f"[{timestamp}] {message}"

        self._log_messages.append(formatted_message)
        self._refresh_log_display()
        self._save_log_to_file(formatted_message)

    def _refresh_log_display(self) -> None:
        self.log_view.clear()

        for message in self._log_messages[-self.MAX_LOG_MESSAGES:]:
            self.log_view.appendPlainText(message)

    @Slot()
    def _clear_log(self) -> None:
        self._log_messages.clear()
        self.log_view.clear()

    # =========================
    # 错误处理
    # =========================

    @Slot(str)
    def _on_error(self, message: str) -> None:
        self._append_log(f"ERR  {message}")
        self.statusBar().showMessage(message)

        if "打开继电器串口失败" in message:
            self._set_relay_connected(False)

        if "打开电源串口失败" in message:
            self._set_power_connected(False)

    # =========================
    # 关闭窗口
    # =========================

    def closeEvent(self, event) -> None:  # type: ignore[override]
        if self._relay_connected:
            self._disconnect_relay()

        if self._power_connected:
            self._disconnect_power()

        if self._relay_thread is not None:
            self._relay_thread.stop()
            self._relay_thread.wait(1200)

        if self._power_thread is not None:
            self._power_thread.stop()
            self._power_thread.wait(1200)

        event.accept()


def main() -> int:
    app = QApplication(sys.argv)
    window = MainWindow()
    window.show()
    return app.exec()


if __name__ == "__main__":
    raise SystemExit(main())